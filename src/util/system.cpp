...
const char * const BITCOIN_CONF_FILENAME = "seizecoin.conf";
...
#else
    const char* pszModule = "seizecoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Seizecoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Seizecoin";
#else
    // Unix
    return pathRet / ".seizecoin";
#endif
#endif
}
